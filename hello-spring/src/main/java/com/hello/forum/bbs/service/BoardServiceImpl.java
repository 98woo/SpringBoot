...
/*
 * @Service : @Controller 와 @Repository 를 연결하는 역할
 * 		- 주로 : Transaction 처리를 담당한다.
 * 		- 업무 로직을 담당한다.
 * 		- 업무 로직
 * 			- 토스
 ...
 * 				------> 업무로직이 정상적으로 종료 되었다면 Commit을 수행 ==>  Transaction.
 ...
 * @Service
 * @Repository
 * 3개의 애너테이션 모두 Spring 이 객체로 생성해서 Bean Container 에 보관한다.
 */
@Service
public class BoardServiceImpl implements BoardService {

	/**
	 * 맴버변수 위에 @Autowired 를 작성하면
	 * Bean Container 에서 맴버변수의 타입과 일치하는 객체를
	 * 찾아서 맴버변수에게 자동으로 할당해준다. (Spring)
	 */
	@Autowired
	...
	public BoardListVO getAllBoard() {
		...
		// BoardDaoImpl 의 getBoardAllCount 를 이용해서 게시글의 건수를 알고 싶고
		int boardCount = this.boardDao.getBoardAllCount();
		
		// BoardDaoImpl 의 getAllBoard 를 이용해서 게시글의 목록을 알고 싶다. 
	...
	public BoardVO getOneBoard(int id, boolean isIncrease) {
		...
		// 게시글 조회한 결과가 null 이라면, 잘못된 접근입니다. 예외를 발생시킨다.
		if (boardVO == null) {
